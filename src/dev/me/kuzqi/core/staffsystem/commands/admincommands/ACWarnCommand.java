 ...
 * @author kuzqi
 */
public class ACWarnCommand implements CommandExecutor {

    private final Main m;
    public ACWarnCommand(Main m) {
        this.m = m;
    }

    @Override
    public boolean onCommand(CommandSender cs, Command cmd, String command, String[] args) {
        if (command.equalsIgnoreCase("acwarn")) {
        	m.getWarnManager().handleACWarnCommand(cs, command, args);
